...
_KrausModel = namedtuple(
    "_KrausModel", ["gate", "params", "targets", "kraus_ops", "fidelity"]
)
...
class KrausModel(_KrausModel):
    ...
    def from_dict(d: Dict[str, Any]) -> "KrausModel":
        ...
        return KrausModel(
            d["gate"], d["params"], d["targets"], kraus_ops, d["fidelity"]
        )
...
class NoiseModel(_NoiseModel):
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "assignment_probs": {
                str(qid): a.tolist() for qid, a in self.assignment_probs.items()
            },
        }
    ...
    def from_dict(d: Dict[str, Any]) -> "NoiseModel":
        ...
        return NoiseModel(
            ...
            assignment_probs={
                int(qid): np.array(a) for qid, a in d["assignment_probs"].items()
            },
        )
...
def _check_kraus_ops(n: int, kraus_ops: Sequence[np.ndarray]) -> None:
    ...
    for k in kraus_ops:
        if not np.shape(k) == (2**n, 2**n):
            raise ValueError(
                "Kraus operators for {0} qubits must have shape {1}x{1}: {2}".format(
                    n, 2**n, k
                )
            )
    ...
    if not np.allclose(kdk_sum, np.eye(2**n), atol=1e-3):
        raise ValueError(
            "Kraus operator not correctly normalized: sum_j K_j^*K_j == {}".format(
                kdk_sum
            )
        )
...
def get_noisy_gate(
    gate_name: str, params: Iterable[ParameterDesignator]
    ...
    if gate_name == "RX":
        ...
        if not isinstance(angle, (int, float, complex)):
            raise TypeError(
                f"Cannot produce noisy gate for parameter of type {type(angle)}"
            )
...
def _decoherence_noise_model(
    ...
    noisy_identities_1q = {
        q: damping_after_dephasing(
            T1.get(q, INFINITY), T2.get(q, INFINITY), gate_time_1q
        )
        for q in all_qubits
    }
    noisy_identities_2q = {
        q: damping_after_dephasing(
            T1.get(q, INFINITY), T2.get(q, INFINITY), gate_time_2q
        )
        for q in all_qubits
    }
    kraus_maps = []
...
def _noise_model_program_header(noise_model: NoiseModel) -> "Program":
    ...
    for k in noise_model.gates:
        ...
        try:
            ...
            print(
                "WARNING: Could not find ideal gate definition for gate {}".format(
                    k.gate
                ),
                file=sys.stderr,
            )
...
class Calibrations:
    ...
    def __init__(self, qc: Optional[QuantumComputer] = None, calibrations: Self = None):
        self.fidelities = {}
    ...
    def _create_2q_dicts(self):
        ...
        for gate in self.two_q_gates:
            fidelity = [
                self.calibrations["2Q"][pair].get("f" + gate, 1.0) for pair in pairs
            ]
    ...
    def change_noise_intensity(self, intensity: float):
        ...
        self.readout_fidelity = self._change_fidelity_by_noise_intensity(
            self.readout_fidelity, intensity
        )
        for name, dic in self.fidelities.items():
            self.fidelities[name] = self._change_fidelity_by_noise_intensity(
                dic, intensity
            )
    ...
    def _change_fidelity_by_noise_intensity(
        self, fidelity: Dict[Any, float], intensity: float
    ):
        """
...
def create_damping_after_dephasing_kraus_maps(
    ...
    matrices = {
        q: damping_after_dephasing(T1.get(q, INFINITY), T2.get(q, INFINITY), gate_time)
        for q in all_qubits
    }
    kraus_maps = []
    for g in gates:
...
def add_single_qubit_noise(
    ...
    :return: A new program with noisy operators.
    """
    prog = create_kraus_maps(
        prog, single_qubit_noise_2Q_gate_name, gate_time_2q, T1, T2
    )
    prog = create_kraus_maps(
        prog, single_qubit_noise_1Q_gate_name, gate_time_1q, T1, T2
    )
    return prog
...
def add_depolarizing_noise(
    prog: Program, fidelities: Dict[str, Dict[str, float]]
) -> Program:
    """
    ...
    for name in fidelities.keys():
        gates = [
            i
            for i in _get_program_gates(prog)
            if i.name == Depolarizing_pre_name + name
        ]
...
def define_noisy_gates_on_new_program(
    ...
    for i in prog:
        if (
            ...
            or damping_after_dephasing_after_2q_gate
        ) and (
            (isinstance(i, Pragma) and i.command == "DELAY")
            or isinstance(i, DelayQubits)
        ):
    ...
    if depolarizing_1q:
        noise_gates[Depolarizing_1Q_gate] = def_gate_to_prog(
            Depolarizing_pre_name + Depolarizing_1Q_gate, 2, new_p
        )
    for gate, val in depolarizing_2q.items():
        if val:
            noise_gates[gate] = def_gate_to_prog(Depolarizing_pre_name + gate, 4, new_p)
    if dec_2q:
        noise_gates[single_qubit_noise_2Q_gate_name] = def_gate_to_prog(
            single_qubit_noise_2Q_gate_name, 2, new_p
        )
    if dec_1q:
        noise_gates[single_qubit_noise_1Q_gate_name] = def_gate_to_prog(
            single_qubit_noise_1Q_gate_name, 2, new_p
        )
    if no_depolarizing:
        ...
        if (
            ...
            if isinstance(i, Gate):
                ...
                if len(targets) == 2:
                    if depolarizing:
                        name = (
                            i.name if i.name in noise_gates.keys() else No_Depolarizing
                        )
...
def add_kraus_maps_to_program(
    ...
    if damping_after_dephasing_after_1q_gate or damping_after_dephasing:
        new_p = add_single_qubit_noise(
            prog=new_p, T1=calibrations.T1, T2=calibrations.T2
        )
